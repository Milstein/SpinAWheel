...
function PrizeWheelGame (params) {
	...
	this.setButtons = function (state) {
		...
		if (state === "start") {
			...
			$("#iw_game_stop_btn").show();
			$("#iw_game_stop_btn").bind("click", function () {
				onStopClick();
			});
		} else if (state === "end") {
	...
	function render () {
		...
		// For stop button logic, we keep rotating by rotationDelta until the stop button is clicked (once the
		// easeInDuration is complete) so the spinner stays at the same speed until the user stops it.
		if (deltaTime < easeInDuration && !stopTriggered) {
			...
			rotationDelta = Math.abs(lastRotation - newRotation);
		} else {
			if (!stopTriggered) {
				if (!$("#iw_game_stop_btn").is(":visible")) {
					thisGame.setButtons("stop");
				}
			}
			if (!stopTriggered || deltaTime < easeInDuration) {
				newRotation += rotationDelta;
			} else {
				newRotation = curveFormula.getValue(deltaTime);
				$("#" + config.id).trigger({type: "slowdown"});
			}
		}

		// If at or after end time, snap to end rotation.
		if (deltaTime >= config.spinDuration && stopTriggered) {
